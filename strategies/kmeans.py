...
def kmeans(n_pool, labeled_idxs, dataset, features, device, i):
    ...
    cluster_learner = KMeans(n_clusters=NUM_QUERY*5)
    cluster_learner.fit(embeddings)
    cluster_idxs = cluster_learner.predict(embeddings)
    centers = cluster_learner.cluster_centers_[cluster_idxs]
    dis = (embeddings - centers)**2
    dis = dis.sum(axis=1)
    score_ordered_idxs = np.array([np.arange(embeddings.shape[0])[cluster_idxs==i][dis[cluster_idxs==i].argmin()] for i in range(NUM_QUERY*5)])
