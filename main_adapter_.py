...
from model import to_train_adapter_ ,get_pred_adapter_
from utils import *
...
while (EXP_ROUND > 0): 
	...
	acc_scores_0 = get_pred_adapter_(eval_dataloader, device, val_features, val_data) # add i=1 to use model from models_dir
	...
	for i in range(1, ITERATION+1):
		...
		## train
		to_train_adapter_(NUM_TRAIN_EPOCH, train_dataloader_i, device, model_i, optimizer_i, lr_scheduler_i)
		
		## iteration i accuracy
		print('\nIter_{} start to get pred.'.format(i))
		acc_scores_i = get_pred_adapter_(eval_dataloader, device, val_features, val_data)
